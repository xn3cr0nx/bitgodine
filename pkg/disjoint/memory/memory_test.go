...
import (
	...
	"github.com/xn3cr0nx/bitgodine/pkg/disjoint/memory"
)
...
var _ = Describe("Testing with Ginkgo", func() {
	...
	It("Should create a new set", func() {
		...
		Expect(arr).To(HaveLen(int(set.GetSize())))
		Expect(set.GetParents()).To(HaveLen(len(arr)))
		Expect(set.GetRanks()).To(HaveLen(len(arr)))
		test := []string{"1f2pY5RYyRwSe7uKNUDaJVpXLKBhgjfd6", "1f2pY5RYyRwSe7uKNUDaJVpXLKBhgjfd6"}
		set2 := memory.NewDisjointSet()
		for _, k := range test {
			set2.MakeSet(k)
		}
		Expect(set2.GetSize()).To(Equal(uint32(1)))
	})
