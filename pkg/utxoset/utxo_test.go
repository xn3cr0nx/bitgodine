...
import (
	...
	. "github.com/xn3cr0nx/bitgodine/pkg/utxoset"
)
...
var _ = Describe("UtxoSet", func() {
	...
	BeforeSuite(func() {
		...
		utxoset = NewUtxoSet(Conf("test.db", true))
	})
	...
	It("Should get a stored utxo set", func() {
		uids, err := utxoset.GetStoredUtxoSet(hash)
		Expect(err).ToNot(HaveOccurred())
		Expect(uids).To(HaveLen(2))
	})
	...
	It("Should get the full utxo set", func() {
		set, err := utxoset.GetFullUtxoSet()
		Expect(err).ToNot(HaveOccurred())
