...
type TestTypeReuseSuite struct {
	suite.Suite
	dgraph *dgo.Dgraph
	db     *badger.DB
}

func (suite *TestTypeReuseSuite) SetupSuite() {
	logger.Setup()
...
func (suite *TestTypeReuseSuite) Setup() {
...
func (suite *TestTypeReuseSuite) TearDownSuite() {
	(*suite.db).Close()
}

// func (suite *TestTypeReuseSuite) TestChangeOutput() {
// 	block, err := btcutil.NewBlockFromBytes(blocks.Block181Bytes)
// 	assert.Equal(suite.T(), err, nil)
...
func (suite *TestTypeReuseSuite) TestVulnerable() {
	block, err := btcutil.NewBlockFromBytes(blocks.Block181Bytes)
	assert.Equal(suite.T(), err, nil)
	testTx := block.Transactions()[1]
	t := &txs.Tx{Tx: *testTx}
...
func TestTypeReuse(t *testing.T) {
	suite.Run(t, new(TestTypeReuseSuite))
}
